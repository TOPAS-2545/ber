...
func marshalFloat(out *forkableWriter, v float64) (err error) {
    switch {
    case math.IsInf(v, 1):
        err = out.WriteByte(byte(0x40))
        if err != nil {
            return
        }
    case math.IsInf(v, -1):
        err = out.WriteByte(byte(0x41))
        if err != nil {
            return
        }        
    case math.IsNaN(v):
        err = out.WriteByte(byte(0x42))
        if err != nil {
            return
        }         
    case v == 0.0:
        if math.Signbit(v) {
            err = out.WriteByte(byte(0x43))
            if err != nil {
                return
            }
        }
...
func marshalBody(out *forkableWriter, value reflect.Value, params fieldParameters) (err error) {
    ...
    case reflect.Float64:
        return marshalFloat(out, v.Float())
